...
| --- | --- | --- | --- | --- | --- | --- | --- |
| You | `maoyouaa` | `feat/i18n-language-toggle` | Commit `e30258c`; merged as PR `#6` | `docs/pr3/i18n-language-toggle.md` | `docs/pr3/evidence/` + `verification-tests.txt` | `complete` | None |
...
| Member 3 | `Min0Nclm` | `fix/user-feedback-task-actions` | Commit `a4d8356`; merged as PR `#4` | `docs/member3-task-feedback-actions.md` | `docs/member3-evidence/dashboard-task-actions-proof.png` + `docs/member3-evidence/authenticated-dashboard-proof.txt` | `complete` | Confirm this GitHub identity matches the correct teammate name in the report |
| Member 4 | `juiSeIn` | `fix/persistence-validation` | Commit `e0fa07b`; merged as PR `#3` | `docs/pr4/persistence-validation.md` | `docs/pr4/evidence/persistence-after-refresh.png` + `docs/pr4/evidence/persistence-check.txt` | `complete` | Confirm this GitHub identity matches the correct teammate name in the report |

## Notes for final report use

- Do not rename GitHub identities to real names unless you have confirmed the
  mapping yourself.
- Member 3 can now be written as `complete`, but the real-name mapping still
  needs confirmation before the final contribution form is filled in.
- Member 4 can now be written as `complete`, but the real-name mapping still
  needs confirmation before the final contribution form is filled in.
